...
import '../data/apis/api_models/get_history_data.dart' hide UserDetails;
...
class _ChatMentorScreenState extends State<ChatMentorScreen> {
  ...
  // ── Mentee info (the person the mentor is chatting with) ─────────────────
  dynamic mentee;
  ...
  void initState() {
    ...
    // Resolve the mentee (the user the mentor is chatting with)
    mentee = widget.appointmentListResult?.userDetails ??
        widget.notificationResult?.userDetails;
    _loadChatHistory();
    _listenToChatStream();
    _initPhase();
  }
  ...
  DateTime? _parseFullDateTime(String s) {
    try {
      // DateTime.parse handles "yyyy-MM-dd HH:mm:ss" natively
  ...
  void _showSessionEndDialog() {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
            'Your chat session has ended. Thank you for your time!',
            style: TextStyle(fontFamily: "Poppins")),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
  ...
  Widget _buildInfoHeader() {
    // mentee is resolved at class level in initState
  ...
  Widget _buildEmptyState() {
    final msg = _phase == _ChatPhase.waitingToStart
        ...
        : _phase == _ChatPhase.inProgress
        ? 'Start the conversation with your mentee!'
